...
async function loadBaseData(year, code, type) {
    ...
    if (!("error" in result)) {
        ...
        <div class="year-block">
            ...
            <span><span class="year">Βαθμός Πρώτου:</span> ${baseFirst[0]}</span>
            <span><span class="year">Βαθμός Τελευταίου:</span> ${baseLast[0]}</span>
            <span><span class="year">Εισακτέοι:</span> ${positions[0]}</span>
        </div>

        <div class="year-block">
            <span class="year-title">${year-1}</span>
            <span><span class="year">Βαθμός Πρώτου:</span> ${baseFirst[1]}</span>
            <span><span class="year">Βαθμός Τελευταίου:</span> ${baseLast[1]}</span>
            <span><span class="year">Εισακτέοι:</span> ${positions[1]}</span>
        </div>
        `;
    }
}
...
// 1. Το κεντρικό γράφημα (Line Chart) που έλειπε και προκαλούσε το σφάλμα
let config = {
    type: 'line',
    data: {
        labels: [],
...
let statsLeftConfig = {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: 'rgba(90, 196, 218, 0.6)',
...
let statsRightConfig = {
    ...
    data: {
        ...
        datasets: [{
            ...
            backgroundColor: 'rgba(237, 87, 132, 0.6)',
            borderColor: 'rgba(237, 87, 132, 1)',
            borderWidth: 2,
            data: []
        }]
    },
...
/*** ΔΙΑΧΕΙΡΙΣΗ ΠΡΟΒΟΛΩΝ (ΔΙΑΓΡΑΜΜΑ - ΠΙΝΑΚΑΣ) ***/
const toggleButtons = document.querySelectorAll('.toggle-btn');
const tabButtons = document.querySelectorAll('.tab-btn');
const chartContainer = document.querySelector('.chart');
const tableContainer = document.querySelector('.table-container:not(.summary-table-container)');
const summaryContainer = document.querySelector('.summary-table-container');
const downloadContainer = document.querySelector('.download-container');
const tableTabs = document.querySelector('.table-tabs'); // Το menu Αναλυτικός/Συγκεντρωτικός
...
toggleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (btn.dataset.target === "chart") {
            
            chartContainer.style.display = "flex";
            tableContainer.style.display = "none";
            summaryContainer.style.display = "none";
            downloadContainer.style.display = "none";
            if (tableTabs) tableTabs.style.display = "none"; 
        } else {
            
            chartContainer.style.display = "none";
            downloadContainer.style.display = "block";
            if (tableTabs) tableTabs.style.display = "flex"; 

          
            tabButtons.forEach(b => b.classList.remove('active'));
            const defaultTab = document.querySelector('.tab-btn[data-table="single"]');
...
/*** ΕΝΑΛΛΑΓΗ ΜΕΤΑΞΥ ΑΝΑΛΥΤΙΚΟΥ ΚΑΙ ΣΥΓΚΕΝΤΡΩΤΙΚΟΥ ΠΙΝΑΚΑ ***/
tabButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        tabButtons.forEach(b => b.classList.remove('active'));
        btn.classList.add('active');

        if (btn.dataset.table === "single") {
            tableContainer.style.display = "block";
            summaryContainer.style.display = "none";
        } else {
            tableContainer.style.display = "none";
...
/*** ΓΕΜΙΣΜΑ ΑΝΑΛΥΤΙΚΟΥ ΠΙΝΑΚΑ ***/
function fillTableFromChart() {
    const tbody = document.querySelector('.table-body');
    if (!tbody) return;
    tbody.innerHTML = ""; 

    config.data.datasets.forEach(dataset => {
        const deptName = dataset.label;
        const bases = dataset.data;
...
/*** ΓΕΜΙΣΜΑ ΣΥΓΚΕΝΤΡΩΤΙΚΟΥ ΠΙΝΑΚΑ (2013-2025) ***/
function fillSummaryTable() {
    ...
    config.data.datasets.forEach(dataset => {
        ...
        tdName.innerText = dataset.label;
        ...
        yearsRange.forEach(year => {
            ...
            if (yearIndex !== -1 && dataset.data[yearIndex] !== null) {
                tdBase.innerText = dataset.data[yearIndex];
            } else {
                tdBase.innerText = "-";
                tdBase.style.color = "#ccc";
            }
...
/*** ΛΗΨΗ EXCEL (ΠΡΟΣΑΡΜΟΣΜΕΝΗ ΣΤΗΝ ΤΡΕΧΟΥΣΑ ΠΡΟΒΟΛΗ) ***/
document.querySelector('.download-btn').addEventListener('click', downloadExcel);

function downloadExcel() {
    let rows = [];
    const isSummaryVisible = (summaryContainer.style.display === "block");

    if (isSummaryVisible) {
        
        let headers = ["Τμήμα"];
        for (let y = 2013; y <= 2025; y++) headers.push(y.toString());
        rows.push(headers);

        config.data.datasets.forEach(dataset => {
            let row = [dataset.label];
            for (let y = 2013; y <= 2025; y++) {
                const idx = dataset.years.indexOf(y);
                row.push(idx !== -1 && dataset.data[idx] !== null ? dataset.data[idx] : "-");
            }
            rows.push(row);
        });
    } else {
       
        rows.push(["Τμήμα", "Έτος", "Βάση"]);
        config.data.datasets.forEach(dataset => {
            dataset.years.forEach((year, index) => {
                rows.push([dataset.label, year, dataset.data[index] !== null ? dataset.data[index] : "-"]);
            });
        });
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Vaseis_Data");
    XLSX.writeFile(workbook, isSummaryVisible ? "vaseis_summary.xlsx" : "vaseis_analytical.xlsx");
}